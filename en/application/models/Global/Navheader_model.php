...
class Navheader_model extends CI_Model {
	...
	function get_headerdata(){
			  ...
			  $this->mongodb->where(array("UserId"=>mongo_id($user_id), "SettingValue"=>"Y"));
			  $subQry = $this->mongodb->get("AccountSettings"); 
			  // Pre-fetch RecordType -> DBTable once (was one Mongo query per section).
			  $rtMap = array();
			  foreach($this->mongodb->get("RecordType") as $rtRow){ if(isset($rtRow['RecordTypeId'])){ $rtMap[(string)$rtRow['RecordTypeId']] = isset($rtRow['DBTable']) ? $rtRow['DBTable'] : null; } }
			  foreach($module_arr as $key=>$value){
				  $module[]    = array("Module"=>$value);;
				  foreach($subQry as $key){
					$record_type_id = $key['RecordTypeId'];
					if(!in_array($record_type_id, $typeids ?? array())){
						$submodule[] = $key;
			  ...
			  return array('module'=> $module,'submod'=>$submodule,"typeIds"=>$typeids, "rec_count"=>$count);
	}
	function getModules(){
		try{
