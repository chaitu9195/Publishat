...
class Deleterecord_model extends CI_Model {
	...
	function delete_record($params){
        ...
        if(count($getTableName ?? array()) > 0)
        {
          ...
          foreach($ids as $id)
          {
            $this->mongodb->where(array("UserId"=>mongo_id($user_id), "RecordId"=>mongo_id($id)));
            $recordData = $this->mongodb->get($tableName);
            if(count($recordData ?? array()) > 0)
            {
              $rdresult = $recordData; 
              ...
              $con->remove(array("RecordId"=>mongo_id($id)));
              if ($qresult){
                $temp = array('19'=>'MedMedicalTestRecords','20'=>'MedPrescriptionMedicine','21'=>'MedFamilyMember','22'=>'MedHealthInsuranceBeneficiary','32'=>'revenue','35'=>'payments');
                $temprectypes = array(19,20,21,22,32,35);
                if(in_array($RecordTypeId, $temprectypes ?? array())){                 
                  $relatedTableName = $temp[$RecordTypeId];
                 
                  $this->mongodb->where(array("ParentRecordId" => mongo_id($id), "UserId" =>mongo_id($user_id)));
                  $checkrelatedrecsqry = $this->mongodb->get($relatedTableName);
                  if(count($checkrelatedrecsqry ?? array()) > 0){
                ...
                return array("status"=>"success","data"=>"Selected record has been deleted successfully");
              }
              else{
                return array("status"=>"failed","data"=>"Not deleted");
              }
            }
          }
  ...
  ----------------------------------------------*/
  function deleltedocumentsfromfolder($user_id,$id,$RecordTypeId){
    //delete attachments from folder
    
    $this->mongodb->where(array("UserId"=> mongo_id($user_id), "RecordId"=> mongo_id($id), "RecordTypeId"=>$RecordTypeId));
