...
class Addrecord_model extends CI_Model
{
    public function add_record($params)
    {
        ...
        if ($result) {
                ...
                for ($i = 0; $i < count($fileids ?? []); $i++) {
                    ...
                    if ($file_id != '') {
                        ...
                        if (count($file_qry ?? []) > 0) {
                            ...
                            $this->mongodb->set(['RecordId' => mongo_id($record_id), 'UploadedFrom' => 'FTR', 'DocumentId' => mongo_id($file_id)]);
            ...
            if (count($docArray ?? []) > 0) {
                ...
                $gridfs->storeUpload('uploadImage', ['_id' => $mid, 'DocumentId' => $mid, 'UserId' => $user_id, 'RecordId' => $record_id, 'RecordTypeId' => $RecordTypeId,'FileType' => $file_extension, 'Notes' => $label]);
            }
            ...
            return ['status' => 'success','rid' => $record_id];
        } else {
    ...
    public function add_sub_record($params)
    {
        ...
        if ($result) {
            ...
            if ($RecordTypeId == 41) {
                $this->mongodb->where(['RecordId' => mongo_id($params['ParentRecordId']), 'UserId' => mongo_id($params['UserId'])]);
            ...
            if (count($fileids ?? []) > 0) {
                for ($i = 0; $i < count($fileids ?? []); $i++) {
                    ...
                    if ($file_id != '') {
                        ...
                        if (count($file_qry ?? []) > 0) {
                            $this->mongodb->where(['_id' => mongo_id($file_id)]);
                            $this->mongodb->set(['RecordId' => mongo_id($record_id), 'UploadedFrom' => 'FTR', 'DocumentId' => mongo_id($file_id),'RecordTypeId' => $RecordTypeId]);
                            $result = $this->mongodb->update('fs.files');
                        }
                    }
                }
