...
class Common_model extends CI_Model {
       ...
       --------------------------------------------*/
		  ...
		  if(!empty($table_name) && $table_name != "failed"){
            ...
            $qry = $this->mongodb->get($table_name);
                foreach($qry as $key){ 
					...
					$key["_id"] = $id;
                  //$data[]  = $key;
				  $this->mongodb->where(array("UserId"=> mongo_id($user_id), "RecordId"=> mongo_id($key['RecordId']), "RecordTypeId"=>$recordTypeId));
           ...
           return array('status'=> "success",'data'=> $data,'count'=>$file_count,'table' =>$table_name);
         } else { 
        ...
        Get Header Count
        ----------------------------------------*/
        function rec_count($recordTypeId) { 
          $table_name = $this->get_table($recordTypeId);  
          $user_id = $this->session->userdata('user_id');
          if(!empty($table_name) && $table_name != "failed"){
			 $this->mongodb->where(array("UserId"=>$user_id));
            $qry = $this->mongodb->get($table_name);
            $count = count($qry ?? array());        
        ...
        -------------------------------------------*/
        function viewrecord_data($params){ 
              ...
              if(!empty($table_name) && $table_name != "failed"){
				...
				$this->mongodb->where(array("RecordId"=>mongo_id($record_id), "RecordTypeId"=>$recordTypeId));
                $qry = $this->mongodb->get("fs.files");
                   foreach($qry as $key) {
                      $file_data[] = $key; 
                   }
                   
                return array('status'=> "success",'data'=> $data,'files'=>$file_data,'table' =>$table_name);
              } else { 
                return array('status'=> "failed");
              }     
        }
	...
	-------------------------------------------*/
	function get_subRecords($params){
	      $recordTypeId = $params['rel_type_id']; 
              ...
              if(!empty($table_name) && $table_name != "failed"){
				  $this->mongodb->where(array("UserId"=>mongo_id($user_id),"ParentRecordId"=>mongo_id($record_id)));
                 $qry = $this->mongodb->get($table_name);
                   foreach($qry as $key){ 
                      $data[]  = $key;
                      $rec_id = $key['RecordId']; 
                      $sub_rec_arr[] = $key; 
					  $this->mongodb->where(array("UserId"=>mongo_id($user_id),"RecordId"=>mongo_id($rec_id), "RecordTypeId"=>$recordTypeId));
                    $qry = $this->mongodb->get("fs.files");
			        $file_count[] = count($qry ?? array()); 
                   foreach($qry as $key) {
                      $file_data[] = $key; 
                   }   
		   }
                  return array('status'=> "success",'data'=> $data,'file_count'=>$file_count,'subfiles'=>$file_data,'sub_recordid'=>$sub_rec_arr);
              } else { 
                return array('status'=> "failed");
              }     
	}
	...
	Delete Sub Record
	------------------------------------------*/
	function deleteSubRecord($params){
	   ...
	   $rec_type_id = $params['rel_type_id'];
          $user_id = $this->session->userdata('user_id');
           ...
           if(!empty($table_name) && $table_name != "failed"){
            $this->mongodb->where(array("UserId"=>mongo_id($user_id), "RecordId"=>mongo_id($record_id),"ParentRecordId"=>mongo_id($parent_rec_id)));
        ...
        Get Table Name 
        -------------------------------------------*/
	function get_table($RecordTypeId){
	...
	function get_tabName($RecordTypeId){
		if(!empty($RecordTypeId)){
            $this->mongodb->where(array("RecordTypeId"=>$RecordTypeId));
			$dbtable = TBL_RECORDTYPE;
			$dbresult = $this->mongodb->get($dbtable);
	...
	function get_moduleName($RecordTypeId){
		if(!empty($RecordTypeId)){
            $this->mongodb->where(array("RecordTypeId"=>$RecordTypeId));
			$dbtable = TBL_RECORDTYPE;
			$dbresult = $this->mongodb->get($dbtable);
        ...
        ------------------------------------------*/
        function get_editrecord_data($record_id, $record_type_id)
        {
          $user_id = $this->session->userdata('user_id');
           $tableName = $this->get_table($record_type_id);
           $this->mongodb->where(array("RecordId" =>mongo_id($record_id), "UserId" =>mongo_id($user_id)));
         $data_qry = $this->mongodb->get($tableName);
	      foreach($data_qry as $key)
	      {
                  $record_data[] = $key; 
              }
			  $this->mongodb->where(array("UserId"=>mongo_id($user_id), "RecordId"=>mongo_id($record_id), "RecordTypeId"=>$record_type_id));
          ...
          return array('data'=>$record_data,"files"=>$file_data);
        }
	...
	---------------------------------------*/
	function delete_single_rec($params) {
            $document_id = $params['docid'];
          $user_id = $this->session->userdata('user_id');
	    $record_id = $params['rid'];
	    $record_type_id = $params['page_refer_id'];
        $this->mongodb->where(array("DocumentId"=>mongo_id($document_id),"UserId"=> mongo_id($user_id), "RecordId"=> mongo_id($record_id), "RecordTypeId"=>$record_type_id));
                  $qry = $this->mongodb->get("fs.files");
	...
	Folder Files
    ----------------------------------------------*/	
	function folderfiles($rec_type_id){
		$user_id = $this->session->userdata('user_id');
		$this->mongodb->order_by(array('TS' => "DESC"));
		$this->mongodb->where(array("RecordTypeId"=>$rec_type_id, "UserId"=>mongo_id($user_id), "Type"=>"File", "UploadedFrom"=>"Folder"));
		$qry = $this->mongodb->get("fs.files");
		foreach($qry as $key)
              {
                  $file_data[] = $key;				  
  ...
  function collaboration($params){ 
    ...
    for($i=0;$i<count($useremail ?? array());$i++){
      $user_email = $useremail[$i];
	  $this->mongodb->where(array("Email"=>$user_email));
	  $qry = $this->mongodb->get("User");
      foreach($qry as $data){
      $userid = $data['UserId'];
	  }
	  $this->mongodb->where(array("touserid"=>mongo_id($userid),"recordid"=>mongo_id($recordid)));
	  $qury = $this->mongodb->get("privileges");	  
      if(count($qury ?? array())> 0)
      {
		 $this->mongodb->where(array("touserid"=>mongo_id($userid),"recordid"=>mongo_id($recordid))); 
		 $this->mongodb->set(array("editprivileges"=>$privileges));
		 $query = $this->mongodb->update("privileges");
		 if($query){
				  $this->mongodb->where(array("UserId"=>mongo_id($user_id),"RecordId"=>mongo_id($recordid)));	
				  $this->mongodb->set(array("Collaboration"=>"collaborated"));
        ...
        if(!empty($userid)){
          if($user_id!=$userid){
			  $qry1 = $this->mongodb->insert("privileges",array("fromuserid"=>mongo_id($user_id),"touserid"=>mongo_id($userid),"dbname"=>$record_type_id,"recordid"=>mongo_id($recordid),"editprivileges"=>$privileges));
			  if($qry1){
				  $this->mongodb->where(array("UserId"=>mongo_id($user_id),"RecordId"=>mongo_id($recordid)));	
				  $this->mongodb->set(array("Collaboration"=>"collaborated"));
				  $update = $this->mongodb->update($table_name);
    ...
    if ($status_code == 1){     
        ...
        return array("status"=>"success","data"=>"Data has been shared.");
      }
      else{
        $status_message = "Error: No emails have been shared. Please try again. ";  
          return array("status"=>"failed","data"=>"No mails shared");
    //    $status_style = $error_status_style;
      }
  }
  function collaboration_rec($recordTypeId){
      $table_name = $this->get_table($recordTypeId); 
      $user_id = $this->session->userdata('user_id');
	  $this->mongodb->where(array("touserid"=>mongo_id($user_id)));
      ...
      foreach($qry as $key)
              {
				  ...
				  $qury = $this->mongodb->get($table_name);
					...
					$this->mongodb->where(array("UserId"=>mongo_id($userid),"RecordId"=>mongo_id($record_id),"RecordTypeId"=>$recordTypeId));
                  $file_count[] = $this->mongodb->count("fs.files");
              }
              return array("shared_data"=>$colaboration_res,"col_files_count"=>$file_count);
  }
