...
*
* PHP 8.4 + ext-mongodb + mongodb/mongodb compatible rewrite.
* Preserves public method names/signatures from the legacy PECL-based library.
*
* @package CodeIgniter
...
* @version Version 2.0 (mongodb extension)
*/

if ( ! class_exists('MongoDB\Client', false))
{
	$__mongo_autoload = FCPATH . 'vendor/autoload.php';
	if (is_file($__mongo_autoload))
	{
		require_once $__mongo_autoload;
	}
	elseif (is_file(APPPATH . '../vendor/autoload.php'))
	{
		require_once APPPATH . '../vendor/autoload.php';
	}
}

require_once APPPATH . 'third_party/mongo_legacy.php';
...
Class Mongo_db{
	...
	* --------------------------------------------------------------------------------
	*
	* Automatically check if the MongoDB extension has been installed/enabled.
	...
	*/
	...
	function __construct($param = array())
	{
		...
		{
			show_error("The MongoDB PHP extension (ext-mongodb) has not been installed or enabled", 500);
		}
		if ( ! class_exists('MongoDB\Client'))
		{
			show_error("The mongodb/mongodb Composer library is not available (MongoDB\\Client missing)", 500);
		}
		$this->CI =& get_instance();
		$this->CI->load->config('mongo_db');
		$this->config = $this->CI->config->item('mongo_db');
		$this->param = is_array($param) ? $param : array();
		$this->connect();
	}
	...
	* --------------------------------------------------------------------------------
	*
	* Modern Client manages connections; no explicit close required.
	*/
	function __destruct()
	{
		// no-op
	...
	*/
	...
	private function connect()
	{
		...
		try
		{
			$no_auth = isset($this->config[$this->activate]['no_auth'])
				&& $this->config[$this->activate]['no_auth'] == TRUE;

			if ($no_auth)
			{
				$uri = "mongodb://{$this->hostname}:{$this->port}/{$this->database}";
			}
			else
			{
				$user = rawurlencode($this->username);
				$pass = rawurlencode($this->password);
				$uri = "mongodb://{$user}:{$pass}@{$this->hostname}:{$this->port}/{$this->database}";
			}

			$this->connect = new MongoDB\Client($uri);
	...
	/**
	* Build writeConcern options for write operations.
	*/
	private function _write_options($extra = array())
	{
		$options = array(
	...
	/**
	* Recursively convert MongoRegex / MongoDate shims to BSON types.
	*/
	private function _convert_legacy_types(&$data)
	{
		if ( ! is_array($data))
		{
			return;
		...
		foreach ($data as $key => &$value)
		{
			...
			{
				$value = $value->toBSONRegex();
			}
			elseif ($value instanceof MongoDate)
			{
				$value = $value->utc;
			}
			elseif ($value instanceof MongoDB\BSON\Regex || $value instanceof MongoDB\BSON\UTCDateTime || $value instanceof MongoDB\BSON\ObjectId)
			{
				// already BSON
			}
			elseif (is_array($value))
			{
				$this->_convert_legacy_types($value);
			}
		}
		unset($value);
	}
	...
	/**
	* Convert a BSON document / array to a plain PHP array (or object).
	*/
	private function _doc_to_array($doc)
	{
		if ($doc === null)
		{
			return null;
	...
	*/
	public function insert($collection = "", $insert = array())
	{
		...
		if (!is_array($insert) || count($insert ?? array()) == 0)
		{
		...
		try
		{
			$this->_convert_legacy_types($insert);
			if ( ! isset($insert['_id']))
			{
				$insert['_id'] = mongo_id();
			}
			$this->db->{$collection}->insertOne($insert, $this->_write_options());
			if (isset($insert['_id']))
			{
				return ($insert['_id']);
			}
	...
	*/
	public function batch_insert($collection = "", $insert = array())
	{
		...
		if (count($insert ?? array()) == 0 || !is_array($insert))
		{
			show_error("Nothing to insert into Mongo collection or insert is not an array", 500);
		}
		try
		{
			foreach ($insert as &$doc)
			{
				...
				$this->_convert_legacy_types($doc);
				if ( ! isset($doc['_id']))
				{
					$doc['_id'] = mongo_id();
				}
			...
			$this->db->{$collection}->insertMany($insert, $this->_write_options());
			if(is_array($insert) && count($insert ?? array()) > 0)
			{
	...
	*/
	...
	{
		if (is_array($wheres) && count($wheres ?? array()) > 0)
		{
	...
	*/
	public function where_in($field = "", $in = array())
	{
		...
		if (is_array($in) && count($in ?? array()) > 0)
		{
			$this->_w($field);
	...
	*/
	public function like($field = "", $value = "", $flags = "i", $enable_start_wildcard = TRUE, $enable_end_wildcard = TRUE)
	{
		...
		$regex = "/$value/$flags";
		$legacy = new MongoRegex($regex);
		$this->wheres[$field] = $legacy->toBSONRegex();
		return ($this);
	}
	...
	*/
	public function get($collection = "")
	{
		...
		try{
			$filter = $this->wheres;
			$this->_convert_legacy_types($filter);
			...
			$this->explain($cursor, $collection);

			$returns = array();
			foreach ($cursor as $document)
			{
				$doc = $this->_doc_to_array($document);
	...
	*/
	...
	{
		if (is_array($where) && count($where ?? array()) > 0)
		{
	...
	*/
	public function find_one($collection = "")
	{
		...
		try{
			...
			$document = $this->db->{$collection}->findOne($filter, $options);
			$this->_clear();
			if(is_null($document))
			{
				return false;
			...
			{
				$document = $this->_doc_to_array($document);
				if ($this->return_as == 'object')
				{
	...
	*/
	public function count($collection = "")
	{
		...
		try
		{
			...
			$options = array();
			if ($this->limit !== NULL && $this->limit != 999999)
			{
		...
		catch (Exception $e)
		{
			if(isset($this->debug) == TRUE && $this->debug == TRUE)
			{
				show_error("MongoDB count failed: {$e->getMessage()}", 500);
			}
			else
			{
				show_error("MongoDB count failed.", 500);
			}
		}
	}
	...
	*/
	public function distinct($collection = "", $field="")
	{
		...
		try
		{
			$filter = $this->wheres;
			$this->_convert_legacy_types($filter);
			$documents = $this->db->{$collection}->distinct($field, $filter);
			$this->_clear();
			if ($this->return_as == 'object')
			{
	...
	*/
	public function update_all($collection = "", $data = array(), $options = array())
	{
		...
		if (is_array($data) && count($data ?? array()) > 0)
		{
	...
	*/
	public function delete($collection = "")
	{
		...
		try
		{
			$filter = $this->wheres;
			$this->_convert_legacy_types($filter);
			$this->db->{$collection}->deleteOne($filter, $this->_write_options());
			$this->_clear();
			return (TRUE);
		}
		catch (Exception $e)
	...
	*/
	public function delete_all($collection = "")
	{
		...
		{
			$this->wheres['_id'] = mongo_id($this->wheres['_id']);
		}
		try
		{
			$filter = $this->wheres;
			$this->_convert_legacy_types($filter);
			$this->db->{$collection}->deleteMany($filter, $this->_write_options());
			$this->_clear();
			return (TRUE);
		}
		catch (Exception $e)
	...
	public function aggregate($collection, $operation)
	{
        if (empty($collection))
	 	...
	 	try
	 	{
	 		...
	 		{
	 			$this->_convert_legacy_types($operation);
	 		}
	 		$documents = $this->db->{$collection}->aggregate($operation)->toArray();
	 		$this->_clear();
	 		if ($this->return_as == 'object')
			{
				return (object)$documents;
	...
	*/
		...
		if (empty($command) || ! is_array($command))
		{
			show_error("no command were specified", 500);
                ...
                try
                {
			$this->_convert_legacy_types($command);
			$cursor = $this->db->command($command);
			$rows = $cursor->toArray();
			$returns = isset($rows[0]) ? $this->_doc_to_array($rows[0]) : array();
	...
	*/
	public function remove_index($collection = "", $keys = array())
	{
		...
		try
		{
			foreach ($keys as $col => $val)
			{
				if ($val == -1 || $val === FALSE || strtolower((string) $val) == 'desc')
				{
					$keys[$col] = -1;
				}
				else
			...
			$indexName = implode('_', $parts);

			$this->db->{$collection}->dropIndex($indexName);
			$this->_clear();
			return TRUE;
		}
		catch (Exception $e)
	...
	*/
	public function list_indexes($collection = "")
	{
		...
		$indexes = array();
		foreach ($this->db->{$collection}->listIndexes() as $index)
		{
			$indexes[] = $this->_doc_to_array($index);
		}
		return $indexes;
	}
	...
	*/
	public function drop_db($database = '')
	{
		...
		try
		{
			$this->connect->dropDatabase($database);
			return (TRUE);
		}
		catch (Exception $e)
		{
	...
	*/
	...
	/**
	* Legacy cursor explain() is not available on modern find cursors.
	* Kept as a safe no-op so call sites do not break.
	*/
