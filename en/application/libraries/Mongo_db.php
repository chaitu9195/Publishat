...
class Mongo_db
{
    ...
    */
    private function _write_options($extra = [])
    {
        $options = [
    ...
    */
    public function batch_insert($collection = '', $insert = [])
    {
        ...
        try {
            ...
            if (is_array($insert) && count($insert ?? []) > 0) {
                $insert_ids = array_map(function ($arr) {
                    return $arr['_id'];
                }, $insert);
                return ($insert_ids);
            } else {
    ...
    */
    public function mul($fields = [], $value = 0)
    {
        $this->_u('$mul');
        if (is_string($fields)) {
    ...
    */
    ...
    public function command($collection, $command = [])
    {
        if (empty($collection)) {
        ...
        try {
            ...
            $rows = $cursor->toArray();
            $returns = isset($rows[0]) ? $this->_doc_to_array($rows[0]) : [];
