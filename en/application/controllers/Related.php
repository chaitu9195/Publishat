      ...
      public function __construct()
	{
      ...
      function editrecord_post() {
		...
		if(!empty($tableName) && $tableName != 'failed' ) {
          ...
          $this->load->view('edit-related', array('data'=> $result['data'][0],'files'=>$result['files'],'folder'=>$folder_files['folder'], "fields"=>$fields, "tabName"=>$tabName, "moduleName"=>$moduleName, "recTypeId"=>$record_type_id, 'parentRecTypeId'=>$parent_record_type_id)); 
        } else {
        $this->load->view('edit-related', array('data'=> 'No Data')); 
        }
      }
      /*
      ...
      function attachfiles_post(){
		...
		if(count($_FILES ?? array())>0) {
			...
			if(!empty($tableName) && $tableName != 'failed' ) {
               $result = $this -> Common_model-> get_editrecord_data($record_id,$record_type_id);
               $this->load->view('edit-related', array('data'=> $result['data'][0],'files'=>$result['files'], "fields"=>$fields, "tabName"=>$tabName, "moduleName"=>$moduleName, "recTypeId"=>$record_type_id, 'parentRecTypeId'=>$parent_record_type_id)); 
             }
           }else  { echo "Failed"; }
