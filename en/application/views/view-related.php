...
var module= '<?=strtolower($moduleName);?>'; 
    $.ajax({
            ...
            data : {rid:id,page_refer_id:type_id,module:module,p_type_id:p_type_id,parentId:pid},
            cache: false, 
            async: false,  
...
function resetform(){
   ...
   $('#error_mail').hide();
   $("#error").hide();
   $("input,select,textarea").css({"border":"1px solid #ccc","background":"#f9f9f9","box-shadow":"inset 0 1px 1px rgba(0,0,0,.075)","transition":"border-color ease-in-out .15s,box-shadow ease-in-out .15s"});
...
.sub_rec_wrapper { padding: 2px 7px;background: #2a5892;width: 25%;color: #fff;cursor: pointer;margin: 3px 5px;  float: left; }
