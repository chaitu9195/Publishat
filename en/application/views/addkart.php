<div class="center_wrapper">
  ...
  <div class="row share_rec" id="share_rec">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-4 col-xs-12">          
        <input type="text" class="form-control" name ='newName'  id="addNew" placeholder="Enter new cart name " style="display:none;">      </div>
    </div>
...
$("#cartName").change(function(){
   ...
   if(cart_name == "addnew") { $("#addNew").show() ; } else { $("#addNew").hide(); } 
});

$("#cartForm").submit(function ( e ) { console.log($('#cartName').val());
       e.preventDefault();
     if( $('#cartName').val() == ''){
        validate("cartName","");
...
displayView("<?=$recTypeId;?>","<?=$files[0]['RecordId']?>","<?=$moduleName;?>","");
});
</script>

<style>
.delete_rec, .share_rec { padding:0px 0 20px 0px; background:#f6f6f6; border:1px solid #eee; margin-top:10px; margin-bottom:20px;width:100%;float:left;}
.delete_rec .top_text, .share_rec .top_text { float:left; background: #3a7f94;color:#fff; width:100%; margin-top:0;margin-bottom: 8px; padding:3px; }
.delete_rec .top_text span, .share_rec .top_text span { padding:4px 3px 2px 12px; float:left; width:90%; }
.delete_body {     margin: 6px 0 0 10px;    float: left;    width: 98%; }
