		...
		if($("#uploadFile").val()) {
			...
			$(this).ajaxSubmit({ 
			   ...
			   success:function (data){
					...
					$("#success").show();
					setTimeout(function(){ getVal('<?=$record_type_id;?>','<?=$moduleName;?>'); },3000);
			   },
			   resetForm: true
			}); 
		    return false; 
			...
			$.ajax({
				...
				$("#msg1").html("Successfully Inserted. Redirecting <i class='fa fa-spinner fa-spin'></i>");
				$("#success").show();
			   setTimeout(function(){rec_count(); getVal('<?=$record_type_id;?>','<?=$moduleName;?>'); },3000);
				}
			});
		}
});
        ...
        success: function (result) { 
           ...
           var modid = '<?=$record_type_id;?>';
               $.ajax({
                 type: 'POST',
                 url: '../web/addrelated',
                 data: {module:mod,mod_id:modid,r_id:result},
                 cache: false, 
                 async: false,  
                 success: function(data){                                        
