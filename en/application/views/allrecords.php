...
<div class="col-sm-12 collaboration"><span>Collaboration Data</span></div>
	  <table class="table table-responsive table-striped table-hover">
	     ...
	     <tbody id="searchable_data">
                 ...
                 <!--<td><span class="btn btn-info" id="share<?=$shared_result[$i]['RecordId'];?>" style="display:none;padding:1px 10px;"> <span class='hidden-xs'>Share</span> <i class="fa fa-share" area-hidden='true' ></i></span></td>-->
               </tr>
            ...
            success: function(data){ 
			  ...
			  var availableTags =  $.parseJSON(data);
				   $.each(availableTags, function(idx, obj) {
					  ...
					  $('#table_data tbody').append("<tr id='"+ value6 +"' onclick=displayView('<?=$recTypeId;?>','"+ value6 +"','<?=strtolower($moduleName);?>','<?=$sub_record_type_id;?>')><td>"+ value1 +"</td><td>" + value2 + "</td>" + value3 + "<td>" + value4 + "</td><td>"+ value5 +"</td></tr>"); 
					});
