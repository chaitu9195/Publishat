...
<div class='row content'>
         <div class="col-sm-8 col-xs-12 left_wrapper"> 
		  ...
		  <div class="sub_records">           
              ...
              <?php }
          } else { ?><div class=''> Related <?=ucfirst($tabName);?> Records not available </div> <?php } ?>
          </div>
		  <?php } ?>
         </div>
         ...
         $.ajax({
            ...
            success: function(data){  
             ...
             $('#error').show(); 
               ...
               $('#error').show(); 
               $('.content').hide(); 
               setTimeout(function(){rec_count();  getVal('<?=$recTypeId;?>','<?=$moduleName;?>'); },4000);
             }
            }
        });
}
        ...
        $.ajax({
            ...
            if(data !='success' )  {                                    
             ...
             $('#error_collaboration').show(); 
               ...
               $('#error_collaboration').show();
               setTimeout( function() { displayView("<?=$recTypeId;?>","<?=$record_id;?>","<?=$moduleName;?>","<?=$sub_type_id;?>") } , 3000);
             }
               //resetform();
            }
        });
...
function sub_view(mainmod_id,related_type_id,recId,parent_recId,module){
	   $.ajax({
                 type: 'POST',
                 url: '../web/relatedview',
                 data: {module:module,modid:related_type_id,main_modid:mainmod_id,rid:recId,ParentId:parent_recId},
                 cache: false, 
                 async: false,  
                 success: function(data){                                        
                  $('#body_content').html(data);
...
function addrelated(recId,modid,mod){
               $.ajax({
                 type: 'POST',
                 url: '../web/addrelated',
                 data: {module:mod,mod_id:modid,r_id:recId},
                 cache: false, 
                 async: false,  
                 success: function(data){                                        
                  $('#body_content').html(data);
...
function subRecDelete(related_type_id,sub_recId,parent_recId,mod){ 
	$.ajax({
            ...
            data : {rec_id:sub_recId,module:mod,rel_type_id:related_type_id,p_rid:parent_recId},
            cache: false, 
            async: false,  
            success: function(data){                                        
                if(data == 'success') {
...
.delete_rec, .share_rec, .colaborate_rec { display:none; padding:0px 0 20px 0px; background:#f6f6f6; border:1px solid #eee; margin-top:10px; margin-bottom:20px;width:100%;float:left;}
.delete_rec .top_text, .colaborate_rec .top_text, .share_rec .top_text { float:left; background: skyblue; width:100%; margin-top:0; padding:3px; }
.delete_rec .top_text span, .share_rec .top_text span,.colaborate_rec .top_text span { padding:4px 3px 2px 12px; float:left; width:90%; }
...
.file_wrapper_view{ height: 83px; position:relative;padding:0;border-bottom:1px solid #eee;margin-bottom:5px;}
...
.sub_records {     float: left;    margin: 5px 0;    width: 100%;    background: #F9f9f9;    padding: 5px 5px 14px 12px;    }
.sub_records .h4 { color:#000; border-bottom:1px solid #ccc;padding:4px 0;float:left; width:100%; }
.sub_rec_wrapper { padding: 2px 7px;background: #2a5892;width: 31.5%;color: #fff;cursor: pointer;margin: 3px 5px;  float: left; }
