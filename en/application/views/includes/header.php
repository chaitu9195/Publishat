    ...
    <div class="navbar-header">
     <div class="settings">
      ...
      <a class="navbar-brand " href="<?php echo base_url();?>web/">  
	      <?php 
	      $domain = $_SERVER['SERVER_NAME'];
          if($domain == "publishat.com" || $domain == "www.publishat.com"){
	      ?>
	 	      <span class="h1 active" style="color:white">Publishat</span>
	      <?php } else if($domain == "mydzvault.com" || $domain == "www.mydzvault.com"){ ?>
...
?>
        <li class="dropdown" id = "<?=strtolower($module);?>" >
          ...
          <?php foreach((array) $submod as $nav){ 
           if($mod['Module'] == $nav['Module']){  
            ...
            <li id = "<?=$nav['Setting'];?>">
               <span class="add-icon">
              <a href="javascript:void(0)" class="text" onclick = "getVal('<?=$nav["RecordTypeId"];?>','<?=strtolower($module);?>')">
                <?=$nav['Setting'];?>  <b title="No of files" id="">(<?=$rec_count[$nav["RecordTypeId"]];?>)</b>
				  <!--<?=$nav['Setting'];?>  <b title="No of files" id=""></b>-->
              </a>
              <a href="javascript:void(0)" title="New Record" class="icon" onclick = "getNew('<?=$nav["RecordTypeId"];?>','<?=strtolower($module);?>')"> 
                  <i class="fa fa-plus-square" aria-hidden="true"></i>
                  </a> <a href="javascript:void(0)" class="icon" onclick ="getFolder('<?=$nav["RecordTypeId"];?>','<?=$nav["Setting"];?>','<?=$module;?>')">
 ...
 if($user_id == "59907aac88424a8f66ca48ef" || $user_id == "59907aac88424a8f66ca4962" || $user_id == "59907ad188424a8f66ca89b1" ){
		  ?>
...
?>
        ...
        <?php  if($module == 'Academic') { ?> <i class="fa fa-graduation-cap" aria-hidden="true"></i> <?php }
          ...
          <ul>
        <?php
         foreach((array) $submod as $nav){ 
          if($mod['Module'] == $nav['Module']){  
             $module = $nav['Module'];
                 if(lcfirst($module) == 'health') { $module ='medical'; }  ?>
            <li>
                  <span href="#" onclick ="getVal('<?=$nav["RecordTypeId"];?>','<?=strtolower($module);?>')">
                     <?=$nav['Setting'];?>  <b title="No of files" id="">(<?=$rec_count[$i];?>)</b>
                  </span>
				  <input type="hidden" id="recordTypeId<?=$nav['RecordTypeId'];?>" value="<?=$rec_count[$i];?>">
                  <span href="#" title="New Record" class="icon" onclick="getNew('<?=$nav["RecordTypeId"];?>','<?=strtolower($module);?>')"> 
                  <i class="fa fa-plus-square" aria-hidden="true"></i>
                  </span>  <span href="#" class="icon" style='margin-right:60px' onclick="getFolder('<?=$nav["RecordTypeId"];?>','<?=$nav["Setting"];?>','<?=$module;?>')"><i class="fa fa-folder " aria-hidden="true"></i></span>
              <script>   </script>
            </li>
          <?php  $i++; } } ?> </ul>
</li>
